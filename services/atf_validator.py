...
BENCHMARK_PATH = Path(__file__).resolve().parent.parent / "data" / "ppac_atf_benchmark.json"
...
class AtfValidator:
    ...
    async def cross_validate_fuel_surcharges(cls) -> dict[str, Any]:
        ...
        async with async_session_maker() as session:
            stmt = select(
                func.avg(FareQuote.fuel_surcharge),
        ...
        r = round(r, 4)
        r2 = round(r ** 2, 4)
        verdict = "STRONG_CONVERGENCE" if r >= 0.85 else ("MODERATE_CONVERGENCE" if r >= 0.6 else "DIVERGENT")
